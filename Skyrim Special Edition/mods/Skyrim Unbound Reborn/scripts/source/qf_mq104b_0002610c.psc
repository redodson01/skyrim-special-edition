...
;FortAttackEnableMarker.Disable() ; switch state of fort to "destroyed" state ; SUR - disabled
...
;FortAttackFXEnableMarker.Enable() ; turn on attack FX (fires etc.) ; SUR - disabled
...
;dunDragonMoundQST.SetStage(20) ; SUR - moved to mq103
; evp Delphine
...
;SetObjectiveCompleted(30) ; SUR - disabled
SetObjectiveDisplayed(40, false) ; SUR - changed to false
SetObjectiveDisplayed(50, false) ; SUR - changed to false
; set summons timer
;kmyQuest.greybeardSummonsTimer = GameDaysPassed.GetValue() + 1 ; SUR - disabled
; start scene
OutroScene.Start()
; enable trigger
OutroTrigger.Enable()
utility.wait(1)
;LearnShoutMsg.Show() ; SUR - disabled
...
SetObjectiveDisplayed(30, false) ; SUR - changed to false
...
;BEGIN CODE
; finish MQ103
;MQ103.setstage(200) ; SUR - disabled
...
; transition to MQ105
;MQ105.setstage(10) ; SUR - disabled
...
IntroScene.Start()
SetObjectiveDisplayed(5, false) ; SUR - changed to false
;END CODE
EndFunction
;END FRAGMENT
...
;SetObjectiveCompleted(5) ; SUR - disabled
SetObjectiveDisplayed(15, false) ; SUR - changed to false
...
;MQ104Shout.ShowAsHelpMessage("Shout", 5, 30, 3) ; SUR - disabled
;END CODE
EndFunction
;END FRAGMENT
...
SetObjectiveDisplayed(20, false) ; SUR - changed to false
...
; SUR - added
GlobalVariable MQ104IntegrityReport = Game.GetFormFromFile(0x855727, "Skyrim Unbound.esp") as GlobalVariable
if SkyrimUnboundOverrideWins
	MQ104IntegrityReport.SetValue(2)
else
	MQ104IntegrityReport.SetValue(1)
endif
;END CODE
EndFunction
;END FRAGMENT
...
; SUR - added property
Bool Property SkyrimUnboundOverrideWins Auto
