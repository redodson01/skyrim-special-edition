...
WordofPower property WordZul auto hidden
WordofPower property WordMey auto hidden
WordofPower property WordGut auto hidden
...
Function StartFX()
	...
	;player is close enough to the word
		...
		GlobalVariable DragonSoulsEnabled = Game.GetFormFromFile(0x0022EC, "Skyrim Unbound.esp") as GlobalVariable
		GlobalVariable UnlockShouts = Game.GetFormFromFile(0x01209B, "Skyrim Unbound.esp") as GlobalVariable
		if !DragonSoulsEnabled.GetValue() || UnlockShouts.GetValue()
			bool shouldUnlockWord = true
			if DragonSoulsEnabled.GetValue()
				if UnlockShouts.GetValueInt() == 2
					shouldUnlockWord = (myWord != myWord01)
				elseif UnlockShouts.GetValueInt() == 3
					shouldUnlockWord = (myWord == myWord01)
				endif
			endif
			if shouldUnlockWord
				Game.UnlockWord(myWord)
			endif
			if myWord == WordZul && (!DragonSoulsEnabled.GetValue() || UnlockShouts.GetValueInt() != 3)
				Game.UnlockWord(WordMey)
				Game.UnlockWord(WordGut)
			endIf
		endif
		
		GlobalVariable WordLearnedGV = Game.GetFormFromFile(0x0022F1, "Skyrim Unbound.esp") as GlobalVariable
		WordLearnedGV.SetValueInt(1)
...
Function StopFX()
	...
	GlobalVariable WordWallsEnabled = Game.GetFormFromFile(0x0022EB, "Skyrim Unbound.esp") as GlobalVariable
...
Event onUpdate()
	GlobalVariable WordWallsEnabled = Game.GetFormFromFile(0x0022EB, "Skyrim Unbound.esp") as GlobalVariable
	GlobalVariable WordWallsTriggered = Game.GetFormFromFile(0x0140F5, "Skyrim Unbound.esp") as GlobalVariable
	...
	while((iInTrigger > 0) && !wordLearned)
		if(isLooking())
			if WordWallsEnabled.GetValue()
				WordWallsTriggered.SetValueInt(1)
				StartFX()
			endIf
