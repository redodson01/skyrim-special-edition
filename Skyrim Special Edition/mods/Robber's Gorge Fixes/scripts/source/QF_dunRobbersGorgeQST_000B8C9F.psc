...
Alias_Sentry.GetActorRef().StartCombat(Game.GetPlayer()) ; in case he is in dialogue with the player
BanditFaction.SetEnemy(PlayerHorseFaction, true, true)
;END CODE
EndFunction
;END FRAGMENT
...
Function Fragment_5()
;BEGIN CODE
if Alias_Sentry.GetActorRef().GetAV("Variable06") > 0 && GetStage() < 20
   ;Player ran away after triggering stage 10 or 11
   Alias_Sentry.GetActorRef().SetAV("Variable06", 0)
endif
;END CODE
EndFunction
;END FRAGMENT
...
if GetStage() < 20
   ...
   if playerRef.HasSpell(DLC1VampireChangeFX) || playerRef.GetRace() == WerewolfBeastRace
      SetStage(40)
   else
