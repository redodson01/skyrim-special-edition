...
Auto state CanHit	
	Event OnTrapHitStart(ObjectReference akTarget, float afXVel, float afYVel, float afZVel, float afXPos, float afYPos, float afZPos, int aeMaterial, bool abInitialHit, int aeMotionType)
		...
		float _afXVel = afXVel
		float _afYVel = afYVel
		if akTarget != PlayerRef && (self as ObjectReference) as PhysicsTrap ; in vanilla PhysicsTrap is only used for falling rocks, PhysicsTrapHit isn't
			_afXVel = 0.0
			_afYVel = 0.0
		endif
		
		if acceptableVelocityDelta(damageVelocityThreshold, _afXVel, _afYVel, afZVel)
